...
export class CustomerCreateComponent implements OnInit {
    ...
    this.customerManagementService.getMethod(url).subscribe(
      (response: any) => {
        ...
        if (this.iscustomerEdit) {
          this.customerGroupForm.get("cafno")?.disable();
        }
        this.customerGroupForm.patchValue({
          earlybillday: this.custData.earlybillday.toString()
        });
    ...
    if (serviceAreaId) {
      ...
      let mvnoId =
        localStorage.getItem("mvnoId") === "1"
          ? this.customerGroupForm.value?.mvnoId
          : Number(localStorage.getItem("mvnoId"));
      const url = `/partnerplans/serviceArea?partnerId=${partnerId}&mvnoId=${mvnoId}&serviceAreaId=${serviceAreaId}&planmode=NORMAL`;
      this.customerManagementService.getMethod(url).subscribe(
        (response: any) => {
    ...
    if (e.value) {
      ...
      this.customerManagementService.getMethod(url).subscribe(
        (response: any) => {
          ...
          this.discountPercentage({ planGroupId: e.value }, true);
        },
        (error: any) => {}
      );
    }
  ...
  planMappingListFormGroup(index): FormGroup {
    ...
    return this.fb.group({
      ...
      QuotaUpdate: [this.planGroupForm.value.skipQuotaUpdate],
          ...
          if (serviceAreaId) {
            ...
            let mvnoId =
              localStorage.getItem("mvnoId") === "1"
                ? this.customerGroupForm.value?.mvnoId
                : Number(localStorage.getItem("mvnoId"));
            let url3 = "/branchManagement/getAllBranchesByServiceAreaId?mvnoId=" + mvnoId;
  ...
  makeEmailAndMobileMandatoryOrNot(actualMvnoId) {
    this.systemService
      .getConfigurationByName("IS_MOBILE_AND_EMAIL_REQUIRED", actualMvnoId)
      .subscribe((res: any) => {
  ...
  customerFormInit(mvnoId?) {
    ...
    let serviceAreaArray = JSON.parse(serviceArea);
    if (serviceAreaArray.length !== 0) {
      this.commondropdownService.filterserviceAreaList(actualMvnoId);
    } else {
    ...
    this.mvnoId != 1
      ...
      : "";
    this.makeEmailAndMobileMandatoryOrNot(actualMvnoId);

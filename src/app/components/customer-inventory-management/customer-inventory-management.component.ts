...
export class CustomerInventoryManagementComponent implements OnInit {
  ...
  selectParentModal: boolean = false;
    minDateTime: Date;
  constructor(
  ...
  replaceInventoryModalOpen(inventory): void {
    // this.callReplaceInventoryMsg(inventory);
  ...
  //   callReplaceInventoryMsg(inventory) {
  //     const url = "/checkinventoryreplacement?custId=" + this.custData.id;
  //     this.customerInventoryManagementService.getMethod(url).subscribe(
  //       (response: any) => {
  ...
  //           this.selOtherItemId = inventory.inOutWardMACMapping[0].itemId;
  //         } else {
  //           this.inventoryReplacementMsg = response.msg;
      ...
      (response: any) => {
        if (response.responseCode == 200) {
           this.messageService.add({
            severity: "success",
            summary: "Success",
            detail: "Inventory rejected successfully",
            icon: "far fa-times-circle"
        });
  ...
  selMacSearchOption(event) {
    this.searchForm.get("searchMacDeatil")?.reset();
    ...
    this.customerInventoryManagementService.getMethod(hierarchyUrl).subscribe(
      (response: any) => {
        if (response.responseCode === 200) {
          ...
          this.netWorkHierarchyName = this.getDeepestChildName(response.data?.[0]) || "Hierarchy";
        } else {
          this.show = false;
          this.messageService.add({
            severity: "info",
  ...
  getDeepestChildName(node: any): string {
    while (node?.children && node.children.length > 0) {
      node = node.children[0]; // Assuming always the first child
    }
  ...
  selectParent(e, type) {
    const url = `/NetworkDevice/customerPortBlock?parentDeviceId=${e.value}&&customerId=${this.custData.id}`;
    this.customerInventoryManagementService.getMethod(url).subscribe(
      (response: any) => {
        if (type == "IN") {
  ...
  onSnSplitterChange(snSplitterId: number, isPreFill = false) {
    ...
    const parentUrl = `/NetworkDevice/customerPortBlock?parentDeviceId=${snSplitterId}&customerId=${this.custData.id}`;
    forkJoin([

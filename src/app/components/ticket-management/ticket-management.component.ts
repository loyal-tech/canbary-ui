...
export class TicketManagementComponent implements OnInit {
  ...
  currentPage = 1;
  ticketConfigitemsPerPage = RadiusConstants.PER_PAGE_ITEMS;
  ...
  findingNameOption: any = [];
  findingRemarkOption: any = [];
  findingName: string = "";
  today: string;
  activeTabListView: number = 0;
  ...
  ngOnInit(): void {
    ...
    this.commondropdownService.mobileNumberLengthSubject$.subscribe(len => {
      if (len) {
        this.ticketGroupForm
          .get("customerAdditionalMobileNumber")
          ?.setValidators([Validators.pattern(`^[0-9]{${len.min},${len.max}}$`)]);

        this.ticketGroupForm.get("customerAdditionalMobileNumber")?.updateValueAndValidity();
      }
    });
    ...
    this.followupForm = this.fb.group({
      ...
      findingRemark: ["", Validators.required],
      remark: ["", Validators.required]
    });
  ...
  addEditTicket(ticketId): void {
    this.submitted = true;
    const mob = String(this.ticketGroupForm.get("customerAdditionalMobileNumber")?.value || "");
    if (mob.trim() !== "") {
      const min = this.commondropdownService.minMobileLength;
      const max = this.commondropdownService.maxMobileLength;

      if (mob.length < min || mob.length > max) {
        this.messageService.add({
          severity: "info",
          summary: "Info",
          detail: `Additional Mobile Number must be between ${min} and ${max} digits.`,
          icon: "far fa-times-circle"
        });
        return;
      }
    ...
    if (this.ticketGroupForm.valid) {
      if (ticketId) {
        ...
        this.updateTicketData.remark = this.createTicketData.remark;
      ...
      (response: any) => {
        ...
        this.statusData.forEach(element => {
          if (
            ...
            this.chnageStatusForm.value.oldStatus === "Resolved" &&
            (element.value === "Closed" || element.value === "Re-open")
          ) {
            this.ChangestatusList.push(element);
  ...
  getTicketById(ticketId) {
    ...
    this.ticketManagementService.getMethod(url).subscribe(
      (response: any) => {
        ...
        this.customersId = this.ticketDeatailData.customersId;

        if (this.customersId) {
          this.getInventoryDetails(this.customersId);
        }
        if (this.customersId) {
          this.getCustomersDetail(this.customersId);
        }
  ...
  getInventoryDetails(customersId) {
    const url = "/customer/getCustNetworkDetail?customerId=" + customersId;
  ...
  searchListTicket(): void {
    this.searchkey = this.searchGroupForm.controls.searchValue.value;
    if (this.activeTabListView == 0) {
      this.currentPageTicketConfig = 1;
      if (!this.searchkey) {
        this.getTicket("");
      } else {
    ...
    } else if (this.activeTabListView == 1) {
      this.currentPageTicketConfig = 1;
      this.getAssignToMeTicketDetails(20);
    } else if (this.activeTabListView == 2) {
      this.currentPageTicketConfig = 1;
      this.getUnpickTicketDetails(20);
    } else {
      this.currentPageTicketConfig = 1;
      this.getAssignToTeamTicketDetails(20);
    }
  }
  ...
  searchTicket() {
    ...
    if (this.searchGroupForm.valid) {
      let data: any = [];
      if (!this.searchkey || this.searchkey !== this.searchGroupForm.controls.searchValue.value) {
        this.currentPageTicketConfig = 1;
      }
      this.searchkey = this.searchGroupForm.controls.searchValue.value;
  ...
  selCustomer(event): void {
    ...
    let mvnoId = this.selectedParentCust?.mvnoId
      ? this.selectedParentCust?.mvnoId
  ...
  selectDeparment(event) {
    const serviceAreaName = this.ticketGroupForm.get("serviceAreaName")?.value;
    if (serviceAreaName) {
      this.getAllTicketReasonCategory(this.ticketServiceList);
    }
  ...
  followupTicket() {
    ...
    if (this.followupForm.valid) {
      const data = {
        ...
        ticketFindingName: this.findingName,
        ticketFindingRemarks: this.followupForm.controls.findingRemark.value
      };
      ...
      this.ticketManagementService.postMethod(url, data).subscribe(
        (response: any) => {
          ...
          //   this.getFollowUpDetailById(this.folloupTicketId);
          this.findingNameOption = [];
          this.findingRemarkOption = [];
          this.findingName = "";
          this.messageService.add({
            severity: "success",
            summary: "Successfully",
  ...
  checkChangeProblemDomain(ticket) {
    ...
    this.ticketManagementService.getMethod(url).subscribe(
      (response: any) => {
        this.getservicesByCustomers(ticket.customersId, () => {
          const serviceId = ticket.ticketServicemappingList?.[0]?.serviceid;
          this.ticketServiceList = [serviceId];
          this.getAllTicketReasonCategory(this.customerServiceData);
          const savedCategoryId = ticket.ticketReasonCategoryId;
          const savedSubCategory = ticket.reasonSubCategoryId;
          this.getSubCategoryByparentCat(savedCategoryId);

          this.problemDomain = savedCategoryId;
          this.subProblemDomain = savedSubCategory;
  ...
  openTicketDetail(ticketId): void {
    ...
    // this.getCustomersDetail(ticketId);
  }
  ...
  getResolutionReasons(event) {
    ...
    if (value === "Raise and Close") {
      ...
      this.ticketGroupForm.controls.finalResolutionId.updateValueAndValidity();
      this.ticketGroupForm.controls.rootCauseReasonId.setValidators(Validators.required);
      this.ticketGroupForm.controls.rootCauseReasonId.updateValueAndValidity();
      const url = "/resolutionReasons/searchBySubCategory/" + subCatId;
      this.ticketManagementService.getMethod(url).subscribe(
        (response: any) => {
          this.resolutionReasonData = response.dataList;
  ...
  getLinkableTickets(ticket) {
    ...
    if (ticket.customersId) {
      let data2 = {
        ...
        ticketReasonSubCategoryId: ticket.reasonSubCategoryId
      };
      data.push(data2);
    }
  ...
  onFileChange(event: any) {
    ...
    // If no valid files remain → reset input
  ...
  getAllTicketReasonCategory(serviceLists: any) {
    ...
    if (serviceLists != null) {
      const selectedType = this.ticketGroupForm.get("caseType")?.value;
      const url = "/ticketReasonCategory/getReasonCategoryByActiveServices?type=" + selectedType;
  ...
  getAssignToMeTicketDetails(size) {
    ...
    if (size) {
      ...
      this.ticketConfigitemsPerPage =
        this.showItemPerPage == 0 ? this.pageITEM : this.showItemPerPage;
    }

    const ticketPagination: any = {
      page: this.currentPageTicketConfig,
      pageSize: this.ticketConfigitemsPerPage,
      sortBy: "createdate",
      sortOrder: 0
  ...
  getTicketCurrentAssigneeData(staffId) {
    ...
    this.adoptCommonBaseService.get(url).subscribe(
      (response: any) => {
        ...
        this.currentStaffId = this.assignStaffData.id;
      },
      (error: any) => {
        this.messageService.add({
          severity: "error",
  ...
  handleChange(event: any) {
    if (event.index == 0) {
      ...
      this.activeTabListView = 0;
    } else if (event.index == 1) {
      this.currentPageTicketConfig = 1;
      this.activeTabListView = 1;
      this.getAssignToMeTicketDetails(20);
    } else if (event.index == 2) {
      this.currentPageTicketConfig = 1;
      this.activeTabListView = 2;
      this.getUnpickTicketDetails(20);
    } else {
      this.activeTabListView = 3;
      this.currentPageTicketConfig = 1;
      this.getAssignToTeamTicketDetails(20);
    }
  }
  ...
  getRemarksFromRemarkType(event) {
    let urldoc = "/ticketFindings/getByRemarkType/" + event.value;
    this.ticketManagementService.getMethod(urldoc).subscribe(
      (response: any) => {
        if (response.responseCode == 200) {
          this.findingNameOption = [];
          this.findingNameOption = response.dataList;
        } else if (response.responseCode == 404) {
          this.messageService.add({
            severity: "error",
            summary: "Error",
  ...
  getRemarkFromFindingName(event) {
    if (event.value) {
      let urldoc = "/ticketFindings/getByFindingsId/" + event?.value?.id;
      this.findingName = event?.value?.name;
      this.ticketManagementService.getMethod(urldoc).subscribe(
        (response: any) => {
          if (response.responseCode == 200) {
            this.findingRemarkOption = [];
            this.findingRemarkOption = response.dataList;
          } else if (response.responseCode == 404) {
            this.messageService.add({
              severity: "error",
              summary: "Error",
  ...
  getservicesByCustomers(id, callback) {
    const url = "/ticketReasonCategory/getAllServiceForSubscribers?customerId=" + id;

    this.customerService.getMethod(url).subscribe((response: any) => {
      this.customerServiceData = response.dataList;
      callback(); // call AFTER data arrives
    });
  }
  onTypeChange() {
    const serviceAreaName = this.ticketGroupForm.get("ticketServicemappingList")?.value;

    if (serviceAreaName) {
      this.getAllTicketReasonCategory(this.ticketServiceList);
      this.getTicketReasonCategory(this.ticketServiceList);
    }
  }
  ...
  allowNumbersOnly(event: any) {
    ...
    this.ticketGroupForm.controls.customerAdditionalMobileNumber.setValue(event.target.value);
  }
  onSearchOptionChange() {
    this.searchGroupForm.controls["searchValue"].setValue("");
    this.searchGroupForm.controls["searchValue"].reset();
  }
}

...
export class CustChargemanagementComponent implements OnInit {
  ...
  constructor(
    ...
    this.chargeManagementAccess = this.loginService.hasPermission(
      this.customerType == "Prepaid"
        ? PRE_CUST_CONSTANTS.PRE_CUST_CAF_CHARGE
        : POST_CUST_CONSTANTS.POST_CUST_CAF_CHARGE
    );
  ...
  ngOnInit(): void {
    ...
    this.systemService.getConfigurationByName("is_installment_allowed").subscribe((res: any) => {
      if (res?.data?.value) {
        ...
        if (this.isInstallemnt) {
          if (this.chargeGroupForm.value.discount > 0) {
            let installmentPrice =
              (this.chargeGroupForm.value.actualprice * this.chargeGroupForm.value.discount) / 100;
            this.installmentPrice = this.chargeGroupForm.value.actualprice - installmentPrice;
            this.getNewPrice(this.installmentPrice);
          } else {
            this.getNewPrice(this.chargeGroupForm.value.actualprice);
          }
        } else {
          if (this.chargeGroupForm.value.discount > 0) {
            let installmentPrice =
  ...
  deletechargeData(SID) {
    ...
    const url = "/deleteCustomerDirectCharge?mvnoId=" + localStorage.getItem("mvnoId");
    this.customerManagementService.postMethod(url, data).subscribe(
      (response: any) => {
        this.getCustChargeDetails("", this.custid);
        this.messageService.add({
  ...
  getParentCustomerData() {
    ...
    const url =
      "/parentCustomers/list/" + this.customerType + "?mvnoId=" + localStorage.getItem("mvnoId");
    this.customerManagementService.postMethod(url, data).subscribe(
      (response: any) => {
